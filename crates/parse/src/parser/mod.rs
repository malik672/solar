...
pub struct Parser<'sess, 'ast> {
    // Hot fields (cache line 1) - accessed frequently in parse_seq_to_before_tokens and other parsing methods
    /// The current token.
    pub token: Token,
    /// The previous token.
    pub prev_token: Token,

    // Hot pointers (cache line 1-2) - frequently accessed
    /// The parser session.
    pub sess: &'sess Session,
    /// The arena where the AST nodes are allocated.
    pub arena: &'ast ast::Arena,

    // Moderately accessed fields (cache line 2-3)
    /// The token stream.
    tokens: std::vec::IntoIter<Token>,

    // Cold fields - rarely accessed, moved to end for better cache utilization
    ...
    // Packed small fields at the end to minimize padding
    /// Current recursion depth (u16 saves 6 bytes vs usize on 64-bit)
    recursion_depth: u16,
    /// Parser state flags packed into a single byte
    flags: ParserFlags,
    // Implicit 5 bytes of padding here for alignment
}
...
impl ParserFlags {
    const IN_YUL: u8 = 1 << 0;
    const IN_CONTRACT: u8 = 1 << 1;

    #[inline]
    fn in_yul(self) -> bool {
        self.bits & Self::IN_YUL != 0
    }

    #[inline]
    fn set_in_yul(&mut self, value: bool) {
        if value {
            self.bits |= Self::IN_YUL;
        } else {
            self.bits &= !Self::IN_YUL;
        }
    }
    ...
    fn in_contract(self) -> bool {
        self.bits & Self::IN_CONTRACT != 0
    }

    #[inline]
    fn set_in_contract(&mut self, value: bool) {
        if value {
            self.bits |= Self::IN_CONTRACT;
        } else {
            self.bits &= !Self::IN_CONTRACT;
        }
    }
...
impl<'sess, 'ast> Parser<'sess, 'ast> {
    ...
    fn check_nr_ident(&mut self) -> bool {
        self.check_or_expected(self.token.is_non_reserved_ident(self.flags.in_yul()), ExpectedToken::Ident)
    }
    ...
    fn in_contract<R>(&mut self, f: impl FnOnce(&mut Self) -> R) -> R {
        let old = self.flags.in_contract();
        self.flags.set_in_contract(true);
        let res = f(self);
        self.flags.set_in_contract(old);
        res
    }
    ...
    fn in_yul<R>(&mut self, f: impl FnOnce(&mut Self) -> R) -> R {
        let old = self.flags.in_yul();
        self.flags.set_in_yul(true);
        let res = f(self);
        self.flags.set_in_yul(old);
        res
    }
    ...
    pub fn with_recursion_limit<T>(
        ...
        let res = if self.recursion_depth as usize > PARSER_RECURSION_LIMIT {
...
impl<'sess, 'ast> Parser<'sess, 'ast> {
    ...
    fn parse_ident_common(&mut self, recover: bool) -> PResult<'sess, Ident> {
        ...
        if ident.is_reserved(self.flags.in_yul()) {
